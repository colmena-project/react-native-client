...
class edit extends Component {
  ...
  showInfoAccount = () => {
    const {myAccountStatus} = this.props;
    return <List.Item title={`${myAccountStatus.data.addresses[0].street}`} description={`${myAccountStatus.data.addresses[0].city}  ${myAccountStatus.data.addresses[0].state}`} left={props => <List.Icon icon="map-marker-outline" />}
   />;
  }
